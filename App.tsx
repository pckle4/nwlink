  ...
  return (
    <div className="relative min-h-screen bg-slate-50 dark:bg-[#0B0F19] text-slate-900 dark:text-white transition-colors duration-300 overflow-x-hidden font-sans selection:bg-indigo-500/30 flex flex-col">
      ...
      <div className="absolute inset-0 pointer-events-none" 
           style={{
             backgroundImage: `radial-gradient(${darkMode ? '#3b82f640' : '#0ea5e930'} 1.5px, transparent 1.5px)`,
             backgroundSize: '32px 32px',
             opacity: 0.8
           }} 
      />
      <div className="absolute inset-0 bg-gradient-to-br from-blue-50/50 via-transparent to-sky-100/50 dark:from-slate-900/80 dark:via-transparent dark:to-indigo-950/40 pointer-events-none" />
      ...
      <header className="fixed top-0 left-0 right-0 z-50 flex justify-center transition-all duration-300 pointer-events-none">
        <div className={cn(
          ...
          <div className={cn("flex items-center gap-3 cursor-pointer group transform-gpu transition-opacity duration-300", isSessionActive && "pointer-events-none opacity-40")} onClick={navigateHome}>
          ...
          <div className={cn("flex items-center gap-2 md:gap-3 transition-opacity duration-300", isSessionActive && "opacity-40 pointer-events-none")}>
              <a href="https://github.com/pckle4/nwlink" target="_blank" rel="noopener noreferrer" className={headerBtnClass} title="Star on GitHub">
               ...
               <a href="mailto:help@nowhile.com" className={headerBtnClass} title="Send Feedback">
      ...
      <main className="relative flex-1 flex flex-col pt-24 md:pt-36 p-4 z-10 w-full max-w-[1400px] mx-auto min-h-[600px] h-full">
         ...
         {view === 'home' && (
           <div className="flex-1 w-full h-full flex flex-col justify-center animate-scale-up">
              {hostId ? <Receiver hostId={hostId} /> : <Sender onToast={addToast} onSessionChange={setIsSessionActive} />}
           </div>
         )}
      </main>
      
      <footer className={cn("py-8 text-center z-10 mt-auto px-4 w-full transition-opacity duration-300", isSessionActive && "opacity-40 pointer-events-none")}>
